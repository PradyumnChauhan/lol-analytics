// API route for spectator (featured games)
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[API Route] Fetching featured games for region: ${region}`);
    console.log(`[API Route] Using API key: ${RIOT_API_KEY.substring(0, 20)}...`);

    // Make request to Riot API (server-side)
    const riotApiUrl = `https://${region}.api.riotgames.com/lol/spectator/v5/featured-games`;
    ...
    console.log(`[API Route] Successfully fetched featured games:`, data.gameList?.length || 0, 'games');
