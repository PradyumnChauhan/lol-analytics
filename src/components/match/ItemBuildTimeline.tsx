...
export function ItemBuildTimeline({ match, playerPuuid, className = '' }: ItemBuildTimelineProps) {
  ...
  // Extract final items
  const finalItems = [
    { slot: 0, itemId: participant.item0, name: getItemName(participant.item0) },
    { slot: 1, itemId: participant.item1, name: getItemName(participant.item1) },
    { slot: 2, itemId: participant.item2, name: getItemName(participant.item2) },
    { slot: 3, itemId: participant.item3, name: getItemName(participant.item3) },
    { slot: 4, itemId: participant.item4, name: getItemName(participant.item4) },
    { slot: 5, itemId: participant.item5, name: getItemName(participant.item5) },
    { slot: 6, itemId: participant.item6, name: getItemName(participant.item6) }, // Trinket
  ...
  const avgGoldPerItem = itemsPurchased > 0 ? totalGold / itemsPurchased : 0;

  // Create estimated timeline
  const itemEvents: ItemEvent[] = [];
  let currentGold = 0;
  let estimatedTime = gameStartTime + 300000; // Start at 5 minutes
  ...
  finalItems.forEach((item, index) => {
    if (item.itemId > 0) {
      ...
      const timeProgress = Math.min(currentGold / totalGold, 0.95); // Cap at 95% of game
  ...
  const buildPhases = calculateBuildPhases(itemEvents, gameDuration);
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        <div className="text-center">
          <div className="text-yellow-400 font-semibold">
            {finalItems.reduce((sum, item) => sum + getItemCost(item.itemId), 0).toLocaleString()}
          </div>
